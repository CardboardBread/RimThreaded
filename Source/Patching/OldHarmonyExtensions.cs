...
using RimThreaded.Patches.ModPatches;
using RimThreaded.Patches.UnityEnginePatches;
using RimThreaded.Patches.Verse.GrammarPatches;
using RimThreaded.Patches.RimWorldPlanetPatches;
using RimThreaded.Patches.VerseAIGroupPatches;
using RimThreaded.Patches.UnityEngineExperimentalRenderingPatches;
using RimThreaded.Patches.VerseAIPatches;
using RimThreaded.Patches.VerseSoundPatches;
using RimThreaded.Patches.VerseProfilePatches;
...
public static class OldHarmonyExtensions
{
    ...
    private static void LoadFieldReplacements()
    {
        ...
        string jsonString = File.ReadAllText(RimThreadedMod.Instance._ReplacementsJsonPath);
            ...
            if (AssembliesToPatch.Contains(assembly.ManifestModule.ScopeName))
            {
                ...
                else
                {
                    ...
                    foreach (Type type in types)
                    {
                        ...
                        allMethods.AddRange(type.GetMethods(AccessTools.all | BindingFlags.DeclaredOnly));
                        allMethods.AddRange(type.GetConstructors(AccessTools.all | BindingFlags.DeclaredOnly));
