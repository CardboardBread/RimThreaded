...
public static class TickList_Patch
{
    [PatchCategory(RimThreadedHarmony.NonDestructiveCategory)]
    [RequireLockPatch(nameof(TickList.RegisterThing))]
    [RequireLockPatch(nameof(TickList.DeregisterThing))]
    ...
    public static IEnumerable<CodeInstruction> Tick(IEnumerable<CodeInstruction> instructions, ILGenerator iLGenerator)
    {
        ...
        while (i < instructionsList.Count)
        {
            if (i + 2 < instructionsList.Count &&
                ...
                instructionsList[i + 2].operand is MethodInfo methodInfo &&
                methodInfo == AccessTools.Method(typeof(Find), "get_TickManager"))
            {
