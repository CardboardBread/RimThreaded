...
namespace RimThreaded.Patches.ModPatches
{
    public class Verb_MeleeAttackCE_Transpile
    {
        ...
        public static void PreApplyMeleeSlaveSuppression(DamageWorker.DamageResult damageResult, Pawn pawn, Verb_MeleeAttack verb_MeleeAttack)
        {
        ...
        public static IEnumerable<CodeInstruction> TryCastShot(IEnumerable<CodeInstruction> instructions, ILGenerator iLGenerator)
        {
            ...
            */
            while (i < instructionsList.Count)
            {
                ...
                if (ci.opcode == OpCodes.Callvirt && (MethodInfo)ci.operand == Method(typeof(Verb_MeleeAttack), nameof(Verb_MeleeAttack.ApplyMeleeDamageToTarget)))
                {
                    yield return ci;
